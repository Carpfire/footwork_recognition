...
def main(pose_dir, out_dir, no_flip):
    for video_name in tqdm(sorted(os.listdir(pose_dir))):
        for side in ['left', 'right']:
            ...
            # Flat case
                video_pose_path = os.path.join(pose_dir, video_name, side)
                video_name = video_name.split('.json.gz')[0]
            else:
                # Nested case
                video_pose_path = os.path.join(
                    pose_dir, video_name, side, 'coco_keypoints.json.gz')
            ...
            if out_dir is not None:
                ...
                store_pickle(os.path.join(out_dir, video_name + '_' + side + '_.emb.pkl'), embs)
        print('Done!')
