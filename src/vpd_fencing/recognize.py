...
DATASETS = ['fx', 'diving48', 'fencing', 'diving48v1', 'tennis', 'fs']
...
def load_fencing_data(dataset, emb_dir, norm, target_fps):
    ...
    video_meta_cache = 'data/sports.cache/fencing.video_meta.pkl'
    if os.path.isdir(dataset_paths.FENCING_VIDEO_DIR):
        video_meta_dict = {
            os.path.splitext(v)[0]: get_metadata(
                os.path.join(dataset_paths.FENCING_VIDEO_DIR, v))
            for v in tqdm(os.listdir(dataset_paths.FENCING_VIDEO_DIR),
                        desc='Loading video metadata')
            if v.endswith('.mp4')
        }
        # cache_video_meta_dict(video_meta_cache, video_meta_dict)
    ...
    classes = ['advancing', 'retreating', 'recover', 'fleche', 'lunge'
            ]

    # def parse_emb_video_name(v):
    #     ''' Takes vid name and splits on the first double underscore then splits the frames'''
    #     clip_name, side, = v.rsplit('__', 1)
    #     # Dont have start and end values 
    #    # video_name, start, end = clip_name.rsplit('_', 2)
    #     #Might just hardcode frames from 0 - 60
    #     return (clip_name, side) 

    # emb_dict = {parse_emb_video_name(k): v for k, v in
    #              load_embs(emb_dir, norm).items()}
    emb_dict = load_embs(emb_dir, norm)
    actions = load_actions('src\\vpd_fencing\\action_dataset\\{}\\all.txt'.format(dataset))
    val_action_ids = load_action_ids(
        'src\\vpd_fencing\\action_dataset\\{}\\val.ids.txt'.format(dataset))
    ...
    for action, label in actions.items():
        ...
        base_video, side, start_frame, end_frame = action.split(':')
        start_frame, end_frame = int(start_frame), int(end_frame)
        embs = emb_dict[''.join([base_video,'__', side ])][0][start_frame:end_frame]
        if len(embs) == 0:
            embs = None
        #print(base_video, side)
       # frame = int(frame)
        for v in emb_dict:

            if (
                    v[0] == base_video and v[1] == side 
            ):
                fps = 30#video_meta_dict[v[-1]].fps
                #mid_frame = frame - v[2]
                start_frame = 0
                end_frame = 60
    ...
    return (to_categories(classes), train_embs, train_labels,
             test_embs, test_labels, None, None, video_label_intervals)
        ...
        for v in emb_dict:
            if (
                ...
                if len(action_embs) > 0:
                    embs = action_embs
                    break
        print(test_prefixes, base_video)
        if base_video.startswith(test_prefixes):
            test_embs[action] = embs
            test_labels[action] = label_idx
        elif action in val_action_ids:
...
def main(emb_dir, dataset, out_dir, algorithm, num_train_examples,
    ...
    elif dataset == 'fencing':
        (categories, train_embs, train_labels, test_embs, test_labels,val_embs, val_labels,
         intervals) = load_fencing_data(dataset, emb_dir, norm, target_fps)
        few_shot_file = 'src\\vpd_fencing\\action_dataset\\fencing\\train_{}_{}.ids.txt'
        if num_epochs is None:
