...
def get_args():
    ...
    parser.add_argument('-d', '--dataset', type=str, required=True,
                        choices=['tennis', 'fs', 'fx', 'diving48', 'fencing'])
...
def get_fencing_dataset(dataset_kwargs):
    tasks = []
    videos = []
    for video_file in tqdm(
            os.listdir(dataset_paths.FENCING_VIDEO_DIR), desc='Loading dataset'
    ):
        ...
        for side in ['left', 'right']:
            side_video_name = '{}__{}'.format(video_name, side)

            video_id = len(videos)
            videos.append(side_video_name)

            count = 0
            for frame_num in range(80):
                img_path_prefix = os.path.join(
                    dataset_paths.FENCING_CROP_DIR, video_name, side,
                    str(frame_num))
                if not os.path.isfile(img_path_prefix + '.png'):
                    continue
                tasks.append((
                    video_id, frame_num , img_path_prefix))
                count += 1
            if count == 0:
                print('{} has no crops'.format(side_video_name))
...
def main(dataset, model_dir, out_dir, model_epoch, flow_img, jitter, no_flip):
    ...
    if dataset == 'tennis':
        ...
        videos, dataset = get_fencing_dataset(dataset_kwargs)
    elif dataset == 'fs':
