...
from vpd_dataset.single_frame import GenericDataset, TennisDataset, PennDataset, FencingDataset
...
DATASETS = ['fencing', 'tennis', 'fs', 'fx', 'penn', 'diving48']
        ...
        with torch.no_grad() if optimizer is None else nullcontext():
            for batch in data_loader:
                ...
                with nullcontext() if scaler is None else autocast():
                    ...
                    #print(f'After Encode: {emb.shape}')
                    gt_emb = batch['emb'].to(device)
                    if hasattr(self, 'fcn_time'):
                        emb = self.fcn_time(emb)
                        #print(f'After Decode: {emb.shape}')
...
def load_dataset(
    ...
    if dataset == 'tennis':
        ...
        if emb_dir is None:
            emb_dir = os.path.join(dataset_paths.FENCING_ROOT_DIR, 'embs')
        if no_test_video:
            dataset_kwargs['exclude_prefixes'] = get_test_prefixes(dataset)
        train_dataset, val_dataset, emb_dim = FencingDataset.load_default(
            emb_dir, dataset_paths.FENCING_CROP_DIR, **dataset_kwargs)
    elif dataset == 'fs':
        if emb_dir is None:
...
def main(
    ...
    rgb_mean_std = RGB_MEAN_STD['resnet' if pretrained or dataset == 'fencing' else dataset]
    ...
    num_load_workers = min(os.cpu_count(), 3)
